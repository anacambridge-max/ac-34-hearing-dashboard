...
export type RejectionPdfData = {
  reason: string; name: string; epic: string; serial: string; ps: string;
  date: string; time: string; venue: string; officer: string;
};
...
export function generateRejectionPdf(actual:RejectionPdfData): Blob {
 ...
 const paragraph=(text:string,spacing=4.5)=>{
  ...
  const lines=doc.splitTextToSize(text,width) as string[];ensureSpace(lines.length*5.1+spacing);
  lines.forEach((line:string,index:number)=>{const words=line.trim().split(/\s+/);const last=index===lines.length-1||words.length<2;
   if(last)doc.text(line,left,y);else{const baseSpace=doc.getTextWidth(" ");const wordWidths=words.reduce((sum,w)=>sum+doc.getTextWidth(w),0);const extra=(width-wordWidths-baseSpace*(words.length-1))/(words.length-1);let x=left;
    words.forEach((word:string,i:number)=>{doc.text(i<words.length-1?word+" ":word,x,y);x+=doc.getTextWidth(word)+baseSpace+(i<words.length-1?extra:0)})}y+=5.1});y+=spacing;
 };
