...
import {FileSpreadsheet,Upload,RefreshCw,Download,Users,FileCheck2,CalendarCheck,AlertTriangle,Search,ChevronDown} from "lucide-react";
...
export default function Page(){
 ...
 const data=useMemo(()=>{
   ...
   const ps=[...byPs.values()].map(x=>({...x,pct:x.scheduled?x.docs/x.scheduled:0,under:x.scheduled>0&&x.docs/x.scheduled<threshold/100,held:/held|completed|complete/i.test(x.status)}));
   return ps;
 },[eci,blo,threshold]);
 ...
 return <main>
  ...
  <section className="uploadGrid">
   <label className="upload"><FileSpreadsheet/><span><b>Upload ECI Report</b><small>{eciName||"Excel / XLSX • Part / Officer data"}</small></span><input type="file" accept=".xlsx,.xls,.csv" onChange={e=>ingest(setEci,setEciName)}/><Upload/></label>
   <label className="upload"><FileCheck2/><span><b>Upload BLO Documents Report</b><small>{bloName||"Excel / XLSX • PS + documents uploaded"}</small></span><input type="file" accept=".xlsx,.xls,.csv" onChange={e=>ingest(setBlo,setBloName)}/><Upload/></label>
  </section>
  ...
  <section className="kpis">{[
   ["Total PS",kpis.ps,Users],["Notices Generated",kpis.generated,FileSpreadsheet],["Notices Delivered",kpis.delivered,FileCheck2],["BLO Documents",kpis.docs,FileCheck2],["Hearing Held",kpis.held,CalendarCheck],["Underperformer PS",kpis.under,AlertTriangle]
  ].map(([label,value,Icon])=><div className="card" key={label as string}><Icon size={19}/><span>{label}</span><strong>{value as number}</strong></div>)}</section>
