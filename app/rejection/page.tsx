...
export default function RejectionPage(){
 ...
 useEffect(()=>{
  ...
  try{
   ...
   doc.setFontSize(14);
   doc.text("ORDER",105,y,{align:"center"});
   y+=11;

   doc.setFontSize(11);
   const fields=[
    ["Elector Name",data.name],
    ["EPIC No.",data.epic],
   ...
   for(const [label,value] of fields){
    ...
    doc.text(lines,valueX,y,{lineHeightFactor:1.2});
    y+=Math.max(6,lines.length*5)+1.5;
   }

   ensureSpace(22);
   doc.setFont("times","bold");
   doc.text("Hearing Date & Venue",left,y);
   doc.setFont("times","normal");
   const venueLines=doc.splitTextToSize(
    [data.date,data.time].filter(Boolean).join("  ")+"  "+(data.venue||"—"),
    valueWidth
   );
